        ...
        for classId, confidence, box in zip(classIds.flatten(),confs.flatten(),bbox):
            ...
            if className in objects:
                ...
                if (draw):
                    ...
                    print(int(box[0]),box[1],box[2],box[3]) # x1 y1 x2 y2
                    #distance_x = box[0] - box[2]
                    #distance_y = box[1] - box[3]
...
def rotate90(clockwise): # rotate 90 deg
	...
	if clockwise == 0: # ccw
		...
		ser.write(b'\x92\x00\x28\xFF\xD8') # rotate counterclockwise
		...
		ser.write(b'\x92\xFF\xD8\x00\x28') # rotate clockwise
		while angleRotated < 90:
			# Send commmand to get angle rotation
			ser.write(b'\x8E\x14') # number 20 = \x14 angle
			time.sleep(.2)
	...
	while (sysRunning_flag):
		...
		# Set to clean mode
			...
			while (robotMode == 1): # Continue to look for table
				#if (getFrontTof() == 1): # If table detected, go to robotMode 2
				#	robotMode = 2
					
				_, frame = cap.read()
				frame = cv2.flip(frame, -1)
				result,objectInfo = getObjects(frame,0.5,0.2,objects=["keyboard"])
